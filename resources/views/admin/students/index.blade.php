...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        ...
        <form method="get" class="flex items-center gap-2">
            <input type="search" name="q" value="{{ $q }}" placeholder="Cari nama / ID / kelas…"
            ...
            <a href="{{ route('admin.students.export', ['q' => $q]) }}"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        <section class="xl:col-span-8">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 md:p-6 h-full flex flex-col">
                ...
                {{-- Toolbar Barcode (baru) --}}
                <form id="barcodeForm" method="POST" action="{{ route('admin.barcodes.students.preview') }}" target="_blank" class="mb-4 flex flex-wrap items-center gap-2">
                    @csrf
                    ...
                    <a href="{{ route('admin.barcodes.students') }}{{ $q ? ('?q='.urlencode($q)) : '' }}"
                       target="_blank"
                       class="inline-flex items-center rounded-lg px-3 py-2 text-xs sm:text-sm font-medium
                ...
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                        <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 12h14M5 16h14"></path>
                        </svg>
                        Daftar Siswa
                    </h2>
                </div>
                    ...
                    <table class="w-full text-left border-collapse text-sm">
                        <thead class="bg-slate-50 dark:bg-slate-900/50 text-xs uppercase text-slate-500 font-semibold">
                            <tr>
                                ...
                                <th class="px-4 py-3 border-b border-slate-100 dark:border-slate-700">Student ID</th>
                                <th class="px-4 py-3 border-b border-slate-100 dark:border-slate-700">Nama</th>
                                <th class="px-4 py-3 border-b border-slate-100 dark:border-slate-700">Grade</th>
                                <th class="px-4 py-3 border-b border-slate-100 dark:border-slate-700">Kelas</th>
                                <th class="px-4 py-3 border-b border-slate-100 dark:border-slate-700">Status</th>
                                <th class="px-4 py-3 border-b border-slate-100 dark:border-slate-700">Barcode</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            @forelse($students as $s)
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                    <td class="px-4 py-2 align-top">
                                        <input type="checkbox" name="ids[]" value="{{ $s->student_id }}" class="row-check accent-indigo-600">
                                    </td>
                                    ...
                                    <td class="px-4 py-2 align-top text-slate-800 dark:text-slate-100">
                                        <div class="font-medium truncate max-w-[220px]" title="{{ $s->name }}">
                                            {{ $s->name }}
                                        </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-8 text-center text-slate-400 text-sm italic">
        ...
        {{-- ============= PANEL KANAN: MANUAL / IMPOR (match Books) ============= --}}
        <section class="xl:col-span-4">
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 md:p-6 h-full relative overflow-hidden">
                ...
                <div id="panelManual" class="hidden relative z-10">
                    ...
                    <form method="post" action="{{ route('admin.students.store') }}" class="grid grid-cols-1 gap-3">
                        @csrf
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400">Student ID</label>
                        ...
                        <div>
                            <label class="text-xs font-semibold text-slate-500 dark:text-slate-400">Nama</label>
                            <input name="name"
                                class="mt-1 w-full rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                <label class="text-xs font-semibold text-slate-500 dark:text-slate-400">Grade</label>
                                <input name="grade"
                                    class="mt-1 w-full rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            </div>
                            <div>
                ...
                <div id="panelImport" class="relative z-10">
                    ...
                    <div class="flex flex-wrap gap-2 mb-4">
                        ...
                        <a href="{{ route('admin.students.sample') }}"
                           class="inline-flex items-center rounded-lg px-3 py-2 text-xs sm:text-sm font-medium bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-100 border border-slate-200 dark:border-slate-700 transition">
                            Download Contoh
                        </a>
                    </div>
...
(() => {
    ...
    // Default: Impor aktif (sama seperti halaman Books)
    setTab('import');
    ...
    function refreshButton() {
        ...
        if (btnPrint) btnPrint.disabled = !any;
    }
